...
class registro_fallas(models.Model):
    ...
    proveedor_short = fields.Char(
        string='Proveedor ',
        compute='_compute_proveedor_short',
        store=True
    )
    ...
    def _compute_proveedor_short(self):
        for record in self:
            if record.proveedor_id:
                ...
                record.proveedor_short = False

    descripcion = fields.Html(
        string='Descripcion de falla'
    )

    descripcion_procesada = fields.Html(
        compute='_compute_descripcion_procesada',
        string="Descripción"
    )
    ...
    def _compute_descripcion_procesada(self):
        for record in self:
            if record.descripcion:
                ...
                chunks = [
                    plain_text[i:i + 50]
                    for i in range(0, len(plain_text), 50)
                ]

                record.descripcion_procesada = '<br/>'.join(chunks)
            else:
                record.descripcion_procesada = False

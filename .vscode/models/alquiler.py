...
class UnidadAlquiler(models.Model):
    ...
    pedidos_count = fields.Integer(compute='_compute_pedidos_count')
    has_pending_orders = fields.Boolean(compute='_compute_pedidos_count', store=False)

    

    def _compute_pedidos_count(self):
        for record in self:
    ...
    repuestos_count = fields.Integer(compute='_compute_repuestos_count')

    def _compute_repuestos_count(self):
        for record in self:

...
export class ContadorDashboardController extends FormController {
    ...
    showFallbackMessage() {
        ...
        if (container) {
            container.innerHTML = '<div class="alert alert-warning text-center m-4">' +
                '<h4>Dashboard en modo de compatibilidad</h4>' +
    ...
    updateStatsDisplay() {
        ...
        if (lastUpdate) {
            ...
            console.warn("Dashboard: Elemento last_update NO encontrado");
        }
    }
    ...
    animateNumber(elementId, targetValue, suffix = '') {
        ...
        if (!element) {
            console.warn("Dashboard: Elemento " + elementId + " NO encontrado para animación");
            return;
        }
    ...
    createEquipoRow(equipo, index) {
        ...
        let contadorColorHtml = '';
        if (equipo.tipo_equipo_detectado === 'color') {
            contadorColorHtml = '<div><i class="bi bi-palette-fill text-info me-1"></i>Color: <strong class="text-info">' + 
                (equipo.contador_color_actual || 0).toLocaleString() + '</strong></div>';
        }
        ...
        tr.innerHTML = '<td class="px-3 py-2">' +
            ...
            '<small class="text-muted"><i class="bi bi-hash"></i> Nº Serie</small>' +
            '</td>' +
            ...
            '<td class="px-3 py-2 text-center">' +
            '<div class="small">' +
            '<div><i class="bi bi-circle-fill text-dark me-1"></i>BN: <strong>' + 
            (equipo.contador_bn_actual || 0).toLocaleString() + '</strong></div>' +
            contadorColorHtml +
            '<div><i class="bi bi-bar-chart-fill text-success me-1"></i>Total: <strong class="text-success">' + 
            (equipo.contador_total_actual || 0).toLocaleString() + '</strong></div>' +
            '</div></td>' +
            '<td class="px-3 py-2 text-center">' +
            '<div class="fw-bold small">' + fechaRelativa + '</div>' +
            '<small class="text-muted"><i class="bi bi-calendar3"></i> ' + 
            new Date(equipo.ultima_actualizacion).toLocaleDateString() + '</small>' +
            '</td>' +
    ...
    renderPagination() {
        ...
        if (!pagination) {
            console.warn("Dashboard: Elemento pagination NO encontrado");
            return;
        }
    ...
    async showDetail(equipoId) {
        console.log("Dashboard: Mostrando detalle del equipo:", equipoId);
        try {
            ...
            console.log("Dashboard: Detalle recibido:", detalle);
    ...
    renderDetailModal(detalle) {
        console.log("Dashboard: Renderizando modal de detalle");
        ...
        let colorSection = '';
        if (detalle.tipo_equipo_detectado === 'color') {
            colorSection = '<div class="col-4">' +
                '<div class="border rounded p-2 mb-2">' +
                '<div class="fs-1"><i class="bi bi-palette-fill text-info"></i></div>' +
        ...
        modalContent.innerHTML = '<div class="row">' +
            '<div class="col-md-6">' +
            '<h6 class="text-primary mb-3"><i class="bi bi-info-circle me-2"></i>Información del Equipo</h6>' +
            '<table class="table table-sm">' +
            '<tr><td><strong><i class="bi bi-building me-1"></i>Cliente:</strong></td><td>' + (detalle.cliente_detectado || 'No detectado') + '</td></tr>' +
            '<tr><td><strong><i class="bi bi-hash me-1"></i>Serie:</strong></td><td class="text-primary">' + (detalle.serie_detectada || 'No detectada') + '</td></tr>' +
            '<tr><td><strong><i class="bi bi-gear me-1"></i>Tipo:</strong></td><td>' + (detalle.tipo_equipo_detectado || 'No detectado') + '</td></tr>' +
            '<tr><td><strong><i class="bi bi-check-circle me-1"></i>Estado:</strong></td><td>' + this.getEstadoBadge(detalle.estado_ultimo) + '</td></tr>' +
            '</table>' +
            '</div>' +
            '<div class="col-md-6">' +
            '<h6 class="text-success mb-3"><i class="bi bi-bar-chart me-2"></i>Contadores Actuales</h6>' +
            '<div class="text-center">' +
            '<div class="row">' +
            '<div class="col-4">' +
            '<div class="border rounded p-2 mb-2">' +
            '<div class="fs-1"><i class="bi bi-circle-fill text-dark"></i></div>' +
            ...
            '<div class="col-4">' +
            '<div class="border rounded p-2 mb-2">' +
            '<div class="fs-1"><i class="bi bi-bar-chart-fill text-success"></i></div>' +
